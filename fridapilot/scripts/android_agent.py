"""Android 逆向分析 Python Agent 脚本

用法:
    python -m fridapilot.scripts.android_agent --target com.example.app --spawn
    python -m fridapilot.scripts.android_agent --target com.example.app --device usb
"""
...
def on_message(message: dict, data) -> None:
    if message["type"] == "send":
        ...
        if t == "activity":
            print(f"  [Activity] {p['method']} → {p['class']}")
        elif t == "crypto":
            print(f"  [Crypto] {p.get('op')} algo={p.get('algo')}")
        elif t == "network":
            print(f"  [Network] {p.get('api')} {p.get('method','')} {p.get('url','')}")
        elif t == "sharedprefs":
            print(f"  [Prefs] {p['op']} {p.get('key')}={p.get('value','')[:80]}")
        elif t == "bypass":
            print(f"  [Bypass] {p.get('detail')}")
        elif t == "intent":
            print(f"  [Intent] {p.get('key')}={p.get('value','')[:80]}")
        else:
            print(f"  [{t}] {p}")
...
def main():
    parser = argparse.ArgumentParser(description="FridaPilot Android Reverse Engineering Agent")
    parser.add_argument("--target", "-t", required=True, help="Package name or PID")
    ...
    print("[*] Monitoring Android app... Press Ctrl+C to stop.\n")
    try:
        if args.timeout > 0:
